...
import { ImageIcon, MessageCircle, Search, Share2, ShoppingBag, Store } from 'lucide-react'
...
export default function PublicShopPage() {
  ...
  useEffect(() => {
    async function loadShop() {
      ...
      if (!shop.online_store_enabled) {
        setBusiness(shop as Business)
        setMessage('Cette boutique n’est pas encore activée.')
        setLoading(false)
        return
      }
  ...
  async function shareShop() {
    ...
    if (navigator.share) {
      await navigator.share({
        ...
        text: `Découvrez la boutique ${business?.name}`,
        url
      })
      return
    }
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-slate-50 px-4">
        <div className="max-w-md rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm">
          <Store className="mx-auto text-slate-300" size={54} />
          <h1 className="mt-4 text-2xl font-black text-slate-950">Boutique indisponible</h1>
          <p className="mt-2 text-sm font-semibold text-slate-500">{message || 'Cette boutique est introuvable.'}</p>
        </div>
      </main>
    )
  }
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      <section className="relative overflow-hidden bg-slate-950">
        <div
          ...
          style={{ backgroundImage: business.banner_url ? `url(${business.banner_url})` : 'none', backgroundColor: primary }}
        />
        ...
        <div className="relative mx-auto max-w-7xl px-5 pb-10 pt-28 md:pb-14 md:pt-36">
          <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            <div className="flex items-end gap-5">
              ...
              <div className="pb-2 text-white">
                ...
                <p className="mt-2 text-sm font-bold text-white/80 md:text-base">
                  {business.slogan || 'Boutique officielle'}
                </p>
                {business.business_address && (
                  <p className="mt-1 text-sm font-semibold text-white/70">{business.business_address}</p>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-3">
              <button
                onClick={shareShop}
                className="inline-flex items-center gap-2 rounded-2xl bg-white/15 px-5 py-3 text-sm font-black text-white backdrop-blur transition hover:bg-white/25"
              >
                <Share2 size={18} />
                Partager
              </button>
      ...
      <section className="mx-auto max-w-7xl px-5 py-8">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full rounded-2xl border border-slate-200 bg-slate-50 py-4 pl-12 pr-4 text-sm font-semibold outline-none focus:border-emerald-300 focus:ring-4 focus:ring-emerald-100"
              />
            </div>
        ...
        {filteredProducts.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-12 text-center">
            ...
            <h2 className="mt-4 text-2xl font-black text-slate-950">Aucun produit disponible</h2>
            <p className="mt-2 text-sm font-semibold text-slate-500">Revenez bientôt ou contactez la boutique sur WhatsApp.</p>
          </div>
        ) : (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {filteredProducts.map((product) => (
              <div key={product.id} className="overflow-hidden rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
                ...
                <div className="mt-5">
                  <p className="mb-2 text-xs font-black uppercase tracking-wide" style={{ color: primary }}>
                    {product.category || 'Produit'}
                  </p>
                  <h3 className="line-clamp-2 min-h-[48px] text-lg font-black leading-snug text-slate-950">
                    {product.name}
                  </h3>

                  <div className="mt-4 flex items-center justify-between gap-3">
                    <p className="text-xl font-black text-slate-950">
                      {formatCfa(Number(product.sell_price || 0))}
                    </p>
                  ...
                  <button
                    ...
                    className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-2xl px-5 py-4 text-sm font-black text-white shadow-lg transition hover:opacity-90"
                    style={{ backgroundColor: primary }}
                  >
                    <MessageCircle size={18} />
        ...
        <div className="mt-10 rounded-3xl border border-slate-200 bg-white p-5 text-center shadow-sm">
          <p className="text-sm font-bold text-slate-500">Boutique propulsée par</p>
          <p className="mt-1 text-xl font-black text-slate-950">CaissePro</p>
        </div>
      </section>
    </main>
  )
