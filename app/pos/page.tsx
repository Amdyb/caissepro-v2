...
import { CreditCard, HandCoins, ImageIcon, Minus, Package, Plus, Search, ShoppingCart, Trash2, UserPlus, Zap } from 'lucide-react'
...
export default function POSPage() {
  ...
  function handleBarcodeSubmit(e: React.FormEvent) {
    ...
    const match = products.find((product) => (product.barcode || '').trim().toLowerCase() === code.toLowerCase())
      || products.find((product) => product.name.toLowerCase().includes(code.toLowerCase()))
  ...
  async function quickAddCustomer() {
    ...
    const { data, error } = await supabase
      .from('customers')
      .insert({ business_id: businessId, full_name: newCustomer.full_name.trim(), phone: newCustomer.phone || null, points: 0, total_spent: 0, debt_balance: 0 })
      .select('id,full_name,phone,points,total_spent,debt_balance')
      .limit(1)
    ...
    if (created) {
      ...
      setMessage('Client ajouté et sélectionné.')
    }
  }
  ...
  async function checkout() {
    ...
    if (paymentMethod === 'credit' && !selectedCustomerId) {
      setMessage('Sélectionnez ou ajoutez un client pour Client Doit.')
      return
    }
    ...
    for (const item of cart) {
      await supabase.from('products').update({ stock: Math.max(Number(item.product.stock || 0) - item.quantity, 0) }).eq('id', item.product.id)
    }

    if (selectedCustomerId && selectedCustomer) {
      await supabase.from('customers').update({
        total_spent: Number(selectedCustomer.total_spent || 0) + subtotal,
        points: Number(selectedCustomer.points || 0) + earnedPoints,
        debt_balance: paymentMethod === 'credit' ? Number(selectedCustomer.debt_balance || 0) + subtotal : Number(selectedCustomer.debt_balance || 0)
  ...
  const Basket = () => (
    <div className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-xl">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h3 className="flex items-center gap-2 text-2xl font-black"><ShoppingCart /> Panier</h3>
          <p className="text-sm font-semibold text-slate-500">{totalItems} article(s)</p>
        </div>
        {cart.length > 0 && <button onClick={() => setCart([])} className="rounded-2xl bg-red-50 px-4 py-3 text-sm font-black text-red-700">Vider</button>}
      </div>

      <div className="mb-4 rounded-3xl bg-slate-50 p-4">
        <label className="text-sm font-black text-slate-700">Client</label>
        <select className="mt-2 w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 font-bold outline-none" value={selectedCustomerId} onChange={(e) => setSelectedCustomerId(e.target.value)}>
          <option value="">Vente sans client</option>
          {customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.full_name} {customer.phone ? `• ${customer.phone}` : ''}</option>)}
        </select>
        <div className="mt-3 grid gap-2 md:grid-cols-[1fr_1fr_auto]">
          <input value={newCustomer.full_name} onChange={(e) => setNewCustomer({ ...newCustomer, full_name: e.target.value })} placeholder="Nouveau client" className="rounded-2xl border border-slate-300 px-4 py-3 text-sm font-bold outline-none" />
          <input value={newCustomer.phone} onChange={(e) => setNewCustomer({ ...newCustomer, phone: e.target.value })} placeholder="Téléphone" className="rounded-2xl border border-slate-300 px-4 py-3 text-sm font-bold outline-none" />
          <button type="button" onClick={quickAddCustomer} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-slate-950 px-4 py-3 text-sm font-black text-white"><UserPlus size={16} />Ajouter</button>
        </div>
      </div>

      <div className="max-h-[38vh] space-y-3 overflow-y-auto pr-1">
        {cart.length === 0 ? <div className="rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center"><ShoppingCart className="mx-auto text-slate-400" /><p className="mt-3 font-black">Panier vide</p></div> : cart.map((item) => (
          <div key={item.product.id} className="rounded-3xl border border-slate-200 p-4">
            <div className="flex items-start justify-between gap-3">
              <div><p className="font-black text-slate-950">{item.product.name}</p><p className="text-xs font-bold text-slate-500">Min: {Number(item.product.minimum_price || 0).toLocaleString('fr-FR')} CFA</p></div>
              <button onClick={() => updateQuantity(item.product.id, 0)} className="rounded-xl p-2 text-red-600"><Trash2 size={16} /></button>
            </div>
            <div className="mt-3 grid grid-cols-[1fr_auto] gap-3">
              <input type="number" value={item.price} onChange={(e) => updatePrice(item.product.id, Number(e.target.value || 0))} className="rounded-2xl border border-slate-300 px-3 py-2 text-sm font-black outline-none" />
              <div className="flex items-center rounded-2xl border border-slate-300 px-2"><button onClick={() => updateQuantity(item.product.id, item.quantity - 1)} className="p-2"><Minus size={14} /></button><span className="px-2 font-black">{item.quantity}</span><button onClick={() => updateQuantity(item.product.id, item.quantity + 1)} className="p-2"><Plus size={14} /></button></div>
            </div>
            <p className="mt-2 text-right font-black">{(item.price * item.quantity).toLocaleString('fr-FR')} CFA</p>
          </div>
        ))}
      </div>
      ...
      <div className="mt-5 rounded-3xl bg-slate-950 p-5 text-white">
        ...
        <div className="mt-5 grid grid-cols-2 gap-3">
          {['cash', 'wave', 'orange_money', 'card', 'credit'].map((method) => <button key={method} onClick={() => setPaymentMethod(method)} className={`rounded-2xl px-4 py-3 text-sm font-black transition ${paymentStyle(method, paymentMethod === method)}`}>{paymentLabel(method)}</button>)}
        </div>
  ...
  return (
    <AppShell title="Caisse POS" subtitle="Produits à gauche, panier visible, client puis encaisser.">
      <div className="mx-auto max-w-[1700px]">
        {message && <div className="mb-5 rounded-2xl bg-emerald-50 p-4 text-sm font-bold text-emerald-700">{message}</div>}
        <div className="grid gap-6 xl:grid-cols-[1fr_460px]">
          <section>
            <div className="sticky top-[88px] z-20 mb-5 rounded-[2rem] border border-slate-200 bg-white/95 p-4 shadow-sm backdrop-blur xl:hidden"><Basket /></div>
            <div className="mb-5 grid gap-4 lg:grid-cols-[1fr_.9fr]">
              <form onSubmit={handleBarcodeSubmit} className="flex gap-3 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="relative flex-1"><Zap className="absolute left-4 top-3.5 text-emerald-600" size={20} /><input ref={barcodeInputRef} className="w-full rounded-2xl border border-slate-300 bg-white py-3 pl-12 pr-4 text-base font-black outline-none" placeholder="Scanner code-barres ou taper un nom..." value={barcodeInput} onChange={(e) => setBarcodeInput(e.target.value)} /></div>
                <button className="rounded-2xl bg-emerald-600 px-5 py-3 font-black text-white">Ajouter</button>
              </form>
              <div className="relative rounded-3xl border border-slate-200 bg-white p-4 shadow-sm"><Search className="absolute left-8 top-7 text-slate-400" size={20} /><input className="w-full rounded-2xl border border-slate-300 bg-white py-3 pl-12 pr-4 font-semibold outline-none" placeholder="Filtrer produits..." value={search} onChange={(e) => setSearch(e.target.value)} /></div>
            </div>
