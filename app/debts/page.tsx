...
import { HandCoins, Plus, Search, Send, UserPlus, WalletCards } from 'lucide-react'
...
export default function DebtsPage() {
  ...
  function whatsappReminder(customer: Customer) {
    ...
    const text = encodeURIComponent(
      `Bonjour ${customer.full_name}, vous avez un solde restant de ${Number(customer.debt_balance || 0).toLocaleString('fr-FR')} CFA chez ${businessName}. Merci.`
    )
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-8 grid gap-5 md:grid-cols-3">
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 text-3xl font-black text-slate-950">{totalDebt.toLocaleString('fr-FR')} CFA</p>
          </div>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 text-3xl font-black text-slate-950">{totalPayments.toLocaleString('fr-FR')} CFA</p>
          </div>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[.85fr_1.15fr]">
          <div className="space-y-8">
            <form onSubmit={addCustomerWithDebt} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="space-y-4">
                ...
                <button disabled={saving} className="w-full rounded-2xl bg-emerald-600 py-4 font-black text-white disabled:opacity-60">
                  Ajouter dette
                </button>
              </div>
            </form>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                <h2 className="text-2xl font-black text-slate-950">Clients débiteurs</h2>
                <p className="text-sm font-semibold text-slate-500">Clients avec solde impayé.</p>
              </div>

              <div className="relative">
                <Search className="absolute left-4 top-3.5 text-slate-400" size={18} />
                <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Rechercher..." className="w-full rounded-2xl border border-slate-300 py-3 pl-11 pr-4 outline-none focus:border-emerald-500 md:w-72" />
              </div>
            </div>
              ...
              {customersWithDebt.map((customer) => (
                <div key={customer.id} className="rounded-3xl border border-slate-200 p-5">
                  <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div>
                      <p className="text-lg font-black text-slate-950">{customer.full_name}</p>
                      <p className="mt-1 text-sm font-semibold text-slate-500">{customer.phone || 'Pas de téléphone'}</p>
                    </div>

                    <div className="text-right">
                      <p className="text-xs font-bold text-red-600">Dette</p>
                      <p className="text-xl font-black text-red-700">{Number(customer.debt_balance || 0).toLocaleString('fr-FR')} CFA</p>
                    </div>
                  </div>

                  <a href={whatsappReminder(customer)} target="_blank" className="mt-4 inline-flex items-center gap-2 rounded-2xl bg-emerald-600 px-5 py-3 text-sm font-black text-white">
